...
public class Z3Helper {
  ...
  /**
   * Combine all conditions using a logical or. In latex you would write something like \$\bigvee${0 \leq i < n} e_i
   * from which the name comes.
   *
   * @param conditions A List containing bool expressions.
   * @return A bool expression that is true iff (at least one expression form conditions is true) or conditions is
   * empty.
   */
  ...
  /**
   * Create a unique name of the passed name. Z3Helper keeps track of declared names through this function and ensures
   * that the returned name was not declared before.
   */
  ...
  static <K> Entry<EnumSort<K>, Map<K, Expr<EnumSort<K>>>> toEnum(
    ...
    EnumSort<K> enumSort = ctx.mkEnumSort(uniqueName(enumName),
  ...
  /**
   * Similar to stream.matchesAny(x -> x == target) this method encodes whether at least one item in the list of options
   * is the target.
   *
   ...
   * @return A boolean expression that is true iff at least one element in the list of options is the target.
   */
  ...
  /**
   * Use this method to check whether all indices from 0 to collection.size() match the given predicate.
   *
   ...
   * @return A boolean expression that is true iff the predicate returns an expression evaluating to true for all
   * indices..
   */
