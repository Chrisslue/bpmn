...
public class Utils {
  ...
  private static <T> void assertContainedAndEqualCount(List<T> list1, List<T> list2) {
    var witnessNotInL2 = list1.stream().filter(
        x -> Collections.frequency(list1, x) != Collections.frequency(list2, x)
    ).findFirst();
    Assertions.assertFalse(
        witnessNotInL2.isPresent(),
        () -> "Element " + witnessNotInL2.get() + " has different frequencies in " + list1 + " and in " + list2);
  }
