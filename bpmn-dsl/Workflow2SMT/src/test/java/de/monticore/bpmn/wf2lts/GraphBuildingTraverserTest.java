...
public class GraphBuildingTraverserTest {
  ...
  public void testNestedGateway() {
    ...
    var startEvent =
        StartEventCollector.of(diagramASt.getProcess().getFlowElementList()).stream()
            .filter(event -> event.getName().equals("Start"))
            .findFirst()
            .orElseThrow();
    var graph = GraphBuildingTraverser.graphOf(startEvent);
    assertEquals(1, graph.getGatewayScopes().size());
    var expectedEdges = List.of(entry("Start", "GatewaySplit"), entry("GatewayMerge", "End"));
  ...
  public void testCyclic() {
    ...
    var expectedEdges =
        List.of(entry("Start", "A"), entry("A", "B"), entry("B", "A"), entry("A", "End"));
    Utils.assertSameEdges(graph, expectedEdges);
  }
