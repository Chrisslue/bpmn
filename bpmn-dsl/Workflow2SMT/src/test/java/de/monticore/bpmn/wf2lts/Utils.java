...
public class Utils {
  ...
  public static void assertSameEdges(
      IntermediateGraphWithScopes graph, List<Entry<String, String>> expectedEdges) {
  ...
  public static void assertSameEdges(
    ...
    List<Entry<String, String>> allEdges =
        graph.getEdges().entrySet().stream()
            .flatMap(
                sourceTargetsEntry ->
            ...
            .map(
                astEntry ->
                    entry(naming.apply(astEntry.getKey()), naming.apply(astEntry.getValue())))
