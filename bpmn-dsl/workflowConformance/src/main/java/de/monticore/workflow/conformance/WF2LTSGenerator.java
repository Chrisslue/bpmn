...
/*
  BPMNNode mkNode(ASTFlowElement node) {
    ...
    if (node instanceof ASTTask) {
      ...
      switch (type) {
        case XOR:
          return ltsBuilder.mkXor(((ASTNamedGateway) node).getName(), children);
        case OR:
          return ltsBuilder.mkOr(((ASTNamedGateway) node).getName(), children);
        case AND:
          return ltsBuilder.mkAnd(((ASTNamedGateway) node).getName(), children);
        default:
