...
import de.monticore.bpmn.timerconditions.AbstractTest;
...
class ISO8601 extends AbstractTest {
  ...
  void testAfter() throws IOException {
    final Optional<ASTAfterPeriodCondition> after = parser.parse_StringAfterPeriodCondition(
        "after P3DT27H13M");
  ...
  void testEvery() throws IOException {
    final Optional<ASTEveryTimeCondition> after = parser.parse_StringEveryTimeCondition(
        "every PT5M");
  ...
  void testEveryWitStart() throws IOException {
    final Optional<ASTEveryTimeCondition> after = parser.parse_StringEveryTimeCondition(
        "start on 2020-02-04 at 13:37, every PT5M");
