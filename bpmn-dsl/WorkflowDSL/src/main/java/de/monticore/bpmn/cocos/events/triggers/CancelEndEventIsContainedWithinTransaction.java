 ...
 */
public class CancelEndEventIsContainedWithinTransaction implements WorkflowASTWFProcessCoCo,
    ...
    WorkflowCollectors.toEndEventsLocal(container).forEach(event -> {
      WorkflowVisitor2 visitor = new WorkflowVisitor2() {
        ...
        public void endVisit(ASTWFEventTriggerCancel node) {
          Log.error(Messages.get("0xWFM2022", event.getName()), event.get_SourcePositionStart(),
  ...
  private void hasNoCancelEndEventSubProcess(final ASTWFSubProcess container) {
    WorkflowCollectors.toEndEventsLocalSubProcess(container).forEach(event -> {
      WorkflowVisitor2 visitor = new WorkflowVisitor2() {
