...
abstract class CommonAntiPatternCoCo extends ProcessGraphCoCo {
   ...
   * @return
   */
  List<ASTFlowElement> getPathEntries(
      final GraphPath<ASTFlowElement, EndpointPair<ASTFlowElement>> path) {
    List<ASTFlowElement> innerNodes = getPathInnerNodes(path);
    
    return innerNodes.stream().filter(node -> node.getPredecessors().stream().filter(
        predecessor -> !path.getVertexList().contains(predecessor)).anyMatch(
            predecessor -> pathFinder.getAllPaths(Sets.newHashSet(innerNodes), Sets.newHashSet(
                predecessor), true, null).isEmpty())).collect(Collectors.toList());
  }
   ...
   * @return
   */
  List<ASTFlowElement> getPathExits(
      final GraphPath<ASTFlowElement, EndpointPair<ASTFlowElement>> path) {
    List<ASTFlowElement> innerNodes = getPathInnerNodes(path);
    
    return innerNodes.stream().flatMap(this::isNonParallelGateway).filter(node -> node
        .getPredecessors().stream().filter(predecessor -> !path.getVertexList().contains(
            predecessor)).anyMatch(predecessor -> pathFinder.getAllPaths(Sets.newHashSet(
                innerNodes), Sets.newHashSet(predecessor), true, null).isEmpty())).collect(
                    Collectors.toList());
  }
  
  private List<ASTFlowElement> getPathInnerNodes(
      final GraphPath<ASTFlowElement, EndpointPair<ASTFlowElement>> path) {
    return path.getVertexList().stream().filter(node -> node != path.getStartVertex() && node
        != path.getEndVertex()).collect(Collectors.toList());
  }
  
  private Stream<ASTWFGateway> isNonParallelGateway(final ASTFlowElement flowNode) {
    return Stream.of(flowNode).flatMap(WorkflowFilters::isGateway).filter(ASTWFGateway::isDiverging)
        .filter(split -> {
          ASTGatewayType type = split.getType();
          return type.isExclusive() || type.isExclusiveEventBased() || type.isInclusive() || type
              .isComplex();
        });
  }
