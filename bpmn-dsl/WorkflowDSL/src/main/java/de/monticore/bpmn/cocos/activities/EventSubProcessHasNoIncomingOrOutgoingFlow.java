 ...
 * Page: 174
 * Description: An Event Sub-Process MUST NOT have any incoming or outgoing Sequence Flows
 */
public class EventSubProcessHasNoIncomingOrOutgoingFlow implements WorkflowASTSubProcessCoCo {
    ...
    public void check(final ASTSubProcess subProcess) {
        if (subProcess.isTriggeredByEvent()) {
            subProcess.streamIncomings().map(SequenceFlow::getSource).forEach(source ->
                    Log.error(Messages.get("0xWFM3001", subProcess.getName()),
                            source.get_SourcePositionStart(), source.get_SourcePositionEnd()));
            subProcess.streamOutgoings().map(SequenceFlow::getTarget).forEach(target ->
                    Log.error(Messages.get("0xWFM3001", subProcess.getName()),
