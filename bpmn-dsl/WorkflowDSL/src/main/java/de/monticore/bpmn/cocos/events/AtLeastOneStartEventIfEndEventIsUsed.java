 ...
 * Page: 238
 * Description: If there is an End Event, then there MUST be at least one Start Event
 */
public class AtLeastOneStartEventIfEndEventIsUsed implements WorkflowASTFlowElementContainerCoCo {
    ...
    public void check(final ASTFlowElementContainer container) {
        ...
        if (endEvents.size() > 0 && startEvents.size() == 0) {
            String formattedEndEvents = endEvents.stream()
                    .map(event -> "\"" + event.getName() + "\"")
                    .collect(Collectors.joining(", "));

            Log.error(Messages.get("0xWFM2006", container.getName(), formattedEndEvents),
