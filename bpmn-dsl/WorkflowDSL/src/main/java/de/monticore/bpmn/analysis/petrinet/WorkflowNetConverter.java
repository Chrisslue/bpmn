 ...
 *
 * The translation handles a single process level at a time. Contained sub-processes must be translated separately.
 *
 * The WF-net does not preserve the BPMN execution semantics if the process or sub-process contains one of:
 * - diverging ior gateway (we do not know which branches should be synchronized later on, only known at runtime)
 * - more than one end event (we do not know which end events should be synchronized)
 * - terminate events
 * - compensate events
...
public class WorkflowNetConverter extends WorkflowLocalVisitor {
    ...
    private Map<ASTPetriNode, Set<ASTFlowNode>> getMapping() {
        return Stream.concat(
                modules.entrySet()
                        .stream()
                        .flatMap(e -> Stream.concat(e.getValue().getPlaces().stream().map(v -> Maps.immutableEntry(v, e.getKey())),
                                e.getValue().getTransitions().stream().map(v -> Maps.immutableEntry(v, e.getKey())))),
                flow2Place.entrySet()
                        .stream()
                        .map(e -> Maps.immutableEntry(e.getValue(), e.getKey().getTarget()))
        )
                .collect(Collectors.toMap(Map.Entry::getKey, e -> Sets.newHashSet(e.getValue()), this::mergeSets));
    }

    private void handleSequenceFlow() {
        // group sequence flows by target node
        Map<ASTFlowNode, List<SequenceFlow>> targets = WorkflowCollectors.toSequenceFlowLocal(localRoot)
                .stream()
                .collect(Collectors.toMap(SequenceFlow::getTarget, Lists::newArrayList, this::mergeLists));

        // handle "uncontrolled flow" (see OMG 13.3.1: Sequence Flow Considerations)
        Maps.filterEntries(targets, this::isUncontrolledFlow).values()
                .forEach(flows -> {
                    ASTPlace p = addPlace("p_flow_merge_" + flows.get(0).getTargetName());
                    flows.forEach(f -> flow2Place.put(f, p));
                });

        Maps.filterEntries(targets, e -> null != e && !isUncontrolledFlow(e)).values()
    ...
    private void handleActivity(final ASTActivity activity, final List<? extends ASTEvent> boundaryEvents) {
        List<EventModule> boundaryModules = boundaryEvents.stream()
                .filter(event -> !isBoundaryCompensationEvent(event))
                .map(modules::get)
                .map(EventModule.class::cast)
                .collect(Collectors.toList());

        ActivityModule module = new ActivityModule(activity, boundaryModules, getInputPlaces(activity), getOutputPlaces(activity));
    ...
    private void handleGateway(final ASTGateway gateway) {
        ...
        if (type.isExclusive() || type.isExclusiveEventBased()) {
            module = new ExclusiveGatewayModule(gateway, getInputPlaces(gateway), getOutputPlaces(gateway));
        } else if (type.isInclusive() || type.isComplex()) {
            module = new InclusiveGatewayModule(gateway, getInputPlaces(gateway), getOutputPlaces(gateway));
        } else {
            module = new ParallelGatewayModule(gateway, getInputPlaces(gateway), getOutputPlaces(gateway));
        }
    ...
    /**
     * Returns the places corresponding to the incoming sequence flow of {@code flowNode} if any or creates a new start place
     * @param flowNode
     * @return
     */
    ...
    /**
     * Returns the places corresponding to the outgoing sequence flow of {@code flowNode} if any or creates a new end place
     * @param flowNode
     * @return
     */
    ...
    public static class Warning {
        ...
        final private ASTFlowNode node;
