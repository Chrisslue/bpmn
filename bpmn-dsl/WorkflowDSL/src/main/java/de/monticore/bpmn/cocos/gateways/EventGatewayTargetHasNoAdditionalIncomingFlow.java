 ...
 * Page: 297
 * Description: Target elements in an Event Gateway configuration MUST NOT have any additional incoming Sequence Flows
 * (other than that from the Event Gateway)
 */
    ...
    public void check(final ASTGateway gateway) {
        if (gateway.getType().isEventBased()) {
            gateway.streamOutgoings()
                    ...
                    .forEach(target -> target.streamIncomings()
                            .map(SequenceFlow::getSource)
                            .filter(source -> !source.equals(gateway))
                            .forEach(source -> Log.error(Messages.get("0xWFM5011", target.getName()),
