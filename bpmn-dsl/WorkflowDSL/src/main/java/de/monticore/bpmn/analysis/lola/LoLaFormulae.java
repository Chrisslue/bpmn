...
public class LoLaFormulae {
  ...
  public static String completion(final WorkflowNet wfNet) { // check liveness of final marking
    return "AG EF " + "(" + wfNet.getSource().getName() + " <= 0" + " AND " + Joiner.on(" AND ")
        .join(wfNet.getPlacesNotSourceOrSink().stream().map(ASTPlace::getName).map(name -> name
            + " <= 0").collect(Collectors.toList())) + " AND " + wfNet.getSink().getName() + " > 0" // proper completion is than guaranteed by safeness check (option to complete +
  ...
  public static String completionReachability(final WorkflowNet wfNet) {
    return "EF (DEADLOCK AND NOT (" + wfNet.getSource().getName() + " <= 0" + " AND " + Joiner.on(
        " AND ").join(wfNet.getPlacesNotSourceOrSink().stream().map(ASTPlace::getName).map(
            name -> name + " <= 0").collect(Collectors.toList())) + " AND " + wfNet.getSink()
                .getName() + " > 0" // proper completion is than guaranteed by safeness check (option to complete +
    // safeness => proper completion)
        + "))";
  }
  ...
  public static String dead(final ASTTransition transition) { // instead of checking liveness directly we check if t is not enabled in the
    // start marking
    return "AG NOT FIREABLE(" + transition.getName() + ")";
  }
  
  public static String live(final ASTTransition transition) { // instead of checking liveness directly we check if t is not enabled in the
    // start marking
