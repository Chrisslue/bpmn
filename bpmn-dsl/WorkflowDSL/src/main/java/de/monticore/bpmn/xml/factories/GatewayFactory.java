...
public class GatewayFactory extends XmlFactory<ASTGateway, TGateway>
  ...
  private void handleGateway(final ASTGateway gateway) {
    ...
    if (type.isExclusive()) {
      ...
      TEventBasedGateway tg =
          create(factory::createTEventBasedGateway, factory::createEventBasedGateway);
      if (gateway.isEmptyIncomings()) {
        tg.setInstantiate(true);
      }
      tg.setEventGatewayType(
          type.isExclusiveEventBased()
              ? TEventBasedGatewayType.EXCLUSIVE
              : TEventBasedGatewayType.PARALLEL);
