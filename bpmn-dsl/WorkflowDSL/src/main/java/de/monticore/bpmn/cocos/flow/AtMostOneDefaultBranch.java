...
public class AtMostOneDefaultBranch implements WorkflowASTFlowBlockCoCo {
    ...
    public void check(final ASTFlowBlock block) {
        final List<ASTFlowBranch> skipBranches = block.streamBranchs().filter(ASTFlowBranch::isDefault).collect(Collectors.toList());
        if (skipBranches.size() > 1) {
            skipBranches.forEach(branch ->
                    Log.error(Messages.get("0xWFM3004"), branch.get_SourcePositionStart())
            );
        }
    }
}
