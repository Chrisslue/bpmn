...
public class ProcessNetIsSound implements WorkflowASTFlowElementContainerCoCo {
    ...
    public void check(final ASTFlowElementContainer root) {
        ...
        if (WorkflowCollectors.toActivities(root).isEmpty()) {
            Log.info("Model does not contain any activities. Skipping soundness check.", getClass().getSimpleName());
            return;
        }

        WorkflowNet wfNet = WorkflowNet.from(root);

        boolean soundTranslation = wfNet.getWarnings().stream().map(WorkflowNetConverter.Warning::getType)
                .noneMatch(WorkflowNetConverter.Warning.Type.UNSOUND_TRANSLATION::equals);
        if (!soundTranslation) {
            Log.warn(Messages.get("0xWFM8002", root.getName()));
        }

        String lolaNet = new PetriNetLoLaPrinter().print(wfNet.getPetriNet(), WorkflowNet.initialMarking(wfNet));
        File lola;
        try {
        ...
        boolean hasNoNonInterruptingBoundaryEvents = WorkflowCollectors.toEvents(root)
                .stream()
    ...
    /**
     * Checks option to complete property for the given workflow-net {@code wfNet}, i. e. if the final marking can be reached from any reachable marking.
     * Option to complete implies: proper completion property, deadlock-freeness
     *
     * @param wfNet workflow-net to be checked
     */
    private void checkOptionToComplete(final WorkflowNet wfNet, final File lolaInput, final ASTFlowElementContainer root,
                                       final Map<ASTPetriNode, Set<ASTFlowNode>> mapping) {
        // LoLa: check liveness of the final marking (a marking is live if it is reachable from any reachable marking)
        // LoLa manual - 4.2.16 Soundness of a workflow net

        try {
            boolean result = new LoLaChecker().formula(LoLaFormulae.completion(wfNet))
                    .input(lolaInput)
                    .check()
                    .getResult();
            if (!result) {
    ...
    /**
     * Checks safeness (1-boundedness) property for the given workflow-net {@code wfNet}, i. e. no place can hold more than one token at the same time
     *
     * @param wfNet workflow-net to be checked
     */
    private void checkSafeness(final WorkflowNet wfNet, final File lolaInput, final ASTFlowElementContainer root,
                               final Map<ASTPetriNode, Set<ASTFlowNode>> mapping) {
        List<ASTFlowNode> unsafeNodes = wfNet.getPlaces().stream().filter(place -> {
            // LoLa manual - 4.2.4 k-boundedness of a place

            try {
                boolean result = new LoLaChecker().formula(LoLaFormulae.safe(place))
                        .input(lolaInput)
                        .check()
                        .getResult();
                return !result;
            } catch (TimeoutException e) {
                Log.warn("Checking safeness for " + root.getName() + " took too long. Skipping.");
            } catch (Exception e) {
                Log.error("Unexpected error while checking safeness for " + root.getName(), e);
            }
            return false;
        })
                .map(mapping::get).filter(Objects::nonNull).flatMap(Collection::stream).distinct()
                .collect(Collectors.toList());

        if (unsafeNodes.size() > 0) {
            isSound = false;
        }
        unsafeNodes.forEach(flowNode ->
                    Log.warn(Messages.get("0xWFM8005", flowNode.getName()), flowNode.get_SourcePositionStart()));
    }

    /**
     * Checks liveness property for the given workflow-net {@code wfNet}, i. e. absence of dead transitions
     *
     * @param wfNet workflow-net to be checked
     */
    private void checkLiveness(final WorkflowNet wfNet, final File lolaInput, final ASTFlowElementContainer root,
                               final Map<ASTPetriNode, Set<ASTFlowNode>> mapping) { // no dead transitions
        List<ASTFlowNode> deadNodes = wfNet.getTransitions().stream().filter(transition -> {
            // LoLa manual - 4.2.8 Dead transition

            try {
                return new LoLaChecker().formula(LoLaFormulae.dead(transition))
                        .input(lolaInput)
                        .check()
                        .getResult();
            } catch (TimeoutException e) {
                Log.warn("Checking liveness for " + root.getName() + " took too long. Skipping.");
            } catch (Exception e) {
                Log.error("Unexpected error while checking safeness for " + root.getName(), e);
            }
            return true ;
        ...
        deadNodes.forEach(flowNode ->
                Log.warn(Messages.get("0xWFM8004", flowNode.getName()), flowNode.get_SourcePositionStart()));
    }
