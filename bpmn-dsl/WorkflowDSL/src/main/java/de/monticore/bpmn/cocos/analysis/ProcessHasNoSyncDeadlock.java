...
public class ProcessHasNoSyncDeadlock extends CommonAntiPatternCoCo {
  ...
  class ProcessHasNoSyncDeadlockVisitor implements WorkflowVisitor2 {
      ...
      if (mergeGateway.isConverging() && mergeGateway.getType().isParallel()) {
        Sets.combinations(mergeGateway.getPredecessors(), 2).stream().map(Lists::newArrayList)
            .forEach(pair -> {
              ...
              Optional<ASTFlowElement> lca = Optional.ofNullable(lcaFinder.getLCA(pair.get(0), pair
                  .get(1)));
              if (!lca.isPresent()) { // parallel gateway used to merge independent branches
                ...
                Stream.of(lca.get()) // TODO check why lcaFinder#getLCASet returns to many nodes
                    .flatMap(WorkflowFilters::isGateway).forEach(splitGateway -> {
                      if (!splitGateway.getType().isParallel() && !splitGateway.getType()
                          .isParallelEventBased()) {
    ...
    private void reportDeadlock(final ASTWFGateway mergeGateway) {
      Log.warn(Messages.get("0xWFM7006", mergeGateway.getName()), mergeGateway
          .get_SourcePositionStart(), mergeGateway.get_SourcePositionEnd());
    }
    
    private void reportDeadlock(final ASTWFGateway splitGateway, final ASTWFGateway mergeGateway) {
    ...
    private void reportPathEntry(final ASTWFGateway splitGateway, final ASTWFGateway mergeGateway,
      ...
      Log.warn(Messages.get("0xWFM7004", entry.getName(), splitGateway.getName()), entry
          .get_SourcePositionStart(), entry.get_SourcePositionEnd());
    }
    
    private void reportPathExit(final ASTWFGateway splitGateway, final ASTWFGateway mergeGateway,
        final ASTFlowElement exit) {
      Log.warn(Messages.get("0xWFM7003", exit.getName(), mergeGateway.getName()), exit
          .get_SourcePositionStart(), exit.get_SourcePositionEnd());
    }
