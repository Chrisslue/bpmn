...
/**
 * A Workflow net (WF-net) is Petri net with a unique source place (input place) and a unique sink place (output place).
 *
 * A "short-circuited" WF-net has an extra transition connecting the sink place to the source place of the WF-net.
 * The short-circuited WF-net is useful for analysis, e.g., when checking liveness.
 *
...
public class WorkflowNet {
     ...
     *
     * The WF-net only includes the given process level. Contained sub-processes must be handled separately.
     *
    ...
    /**
     * Creates the initial marking for a WF-net, i.e., i.e. a marking that has only a token at the source place.
     *
    ...
    /**
     * Returns the short-circuit transition that connects the sink to source place, if it was previously added.
     *
     * @see WorkflowNet#connectSinkToSource()
     *
     * @return optional containing the short-circuit transition or empty optional if it has not been added
     */
    ...
    /**
     * Returns the warnings generated during translation from the BPMN process or sub-process into the WF-net.
     *
     * @return the warnings
     */
