 ...
 * Page: 248
 * Description: An End Event MUST be a target for a Sequence Flow. An End Event MAY have multiple incoming Sequence Flows
 */
public class EndEventHasOneOrMoreIncomingFlows implements WorkflowASTEventCoCo {
    ...
    public void check(final ASTEvent event) {
        if (event.isEnd() && event.isEmptyIncomings()) {
            Log.error(Messages.get("0xWFM2009", event.getName()),
