...
public class ProcessHasNoLackOfSync extends CommonAntiPatternCoCo implements WorkflowInheritanceVisitor {
                    ...
                    .forEach(pair -> {
                        ...
                        Stream.of(lcaFinder.getLCA(pair.get(0), pair.get(1))) // TODO check why lcaFinder#getLCASet returns to many nodes
                                ...
                                .forEach(splitGateway -> {
                                    if (!splitGateway.getType().isExclusive() && !splitGateway.getType().isExclusiveEventBased()) {
    ...
    private void reportPathEntry(final ASTGateway splitGateway, final ASTGateway mergeGateway, final ASTFlowNode entry){
        Log.warn(Messages.get("0xWFM7005", entry.getName(), splitGateway.getName()),
