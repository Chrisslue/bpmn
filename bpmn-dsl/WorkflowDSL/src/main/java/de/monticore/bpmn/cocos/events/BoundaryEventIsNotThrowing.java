 ...
 * Page: 249
 * Description: An Intermediate Event that is attached to the boundary of an Activity can only be used to “catch” the Event trigger.
 */
public class BoundaryEventIsNotThrowing implements WorkflowASTEventCoCo {
    ...
    public void check(final ASTEvent event) {
        if (event.isBoundary() && event.isThrow()) {
            Log.error(Messages.get("0xWFM2015", event.getName()),
