 ...
 * Page: 297
 * Description: Event-Based Gateways are configured by having outgoing Sequence Flows target an Intermediate Event or a
...
public class EventGatewayHasValidTarget implements WorkflowASTGatewayCoCo, WorkflowVisitor {
    ...
    public void check(final ASTGateway gateway) {
        if (gateway.getType().isEventBased()) {
            gateway.streamOutgoings()
                    ...
                    .forEach(event -> Log.error(Messages.get("0xWFM5007", event.getName(), gateway.getName()),
    ...
    private boolean isInvalidTarget(final ASTFlowNode flowNode) {
        WorkflowFilter<ASTFlowNode> filter = new WorkflowFilter<ASTFlowNode>(flowNode){
            @Override
