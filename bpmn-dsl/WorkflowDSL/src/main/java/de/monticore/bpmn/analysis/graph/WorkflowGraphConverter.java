...
public class WorkflowGraphConverter extends WorkflowLocalInheritanceVisitor {
    ...
    public void visit(final ASTFlowNode flowNode) {
        ...
        flowNode.streamIncomings().map(SequenceFlow::getSource).forEach(source -> graph.putEdge(source, flowNode));
        flowNode.streamOutgoings().map(SequenceFlow::getTarget).forEach(target -> graph.putEdge(flowNode, target));
