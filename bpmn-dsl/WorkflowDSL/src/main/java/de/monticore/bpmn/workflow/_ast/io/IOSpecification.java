...
public class IOSpecification {
   ...
   */
  public static IOSpecification from(
      final ASTIOSpecification ast, final IWorkflowScope enclosingScope) {
            ...
            astInputSet
                ...
                .forEach(
                    astInput -> {
                      ...
                      enclosingScope
                          .resolveDataObject(inputName)
                          .ifPresent(inputCandidate::setSourceSymbol);

                      if (!inputs.containsKey(
                          inputCandidate)) { // the same data input does not yet exist
            ...
            astOutputSet
                ...
                .forEach(
                    astOutput -> {
                      ...
                      enclosingScope
                          .resolveDataObject(outputName)
                          .ifPresent(outputCandidate::setTargetSymbol);

                      if (!outputs.containsKey(
                          outputCandidate)) { // the same data output does not yet exist
    ...
    return new IOSpecification(
        inputSets.values(), outputSets.values(), inputs.values(), outputs.values());
  }
