 ...
 * The Intermediate Event MUST be a source for a Sequence Flow. Multiple Sequence Flows MAY originate from an Intermediate Event.
 * If the Intermediate Event is used within normal flow:
 * Intermediate Events MUST be a target of a Sequence Flow. An Intermediate Event MAY have multiple incoming Sequence Flows.
 */
public class IntermediateEventHasOneOrMoreIncomingFlows implements WorkflowASTEventCoCo {
    ...
    public void check(final ASTEvent event) {
        if (event.isIntermediate() && !event.isBoundary() && event.isEmptyIncomings()) {
            Log.error(Messages.get("0xWFM2020", event.getName()),
