...
class IOSpecificationFactory {
    ...
    public static TInputOutputSpecification makeXml(final IOSpecification ioSpec, final String prefix) {
    ...
    private TInputOutputSpecification buildXml(final IOSpecification ioSpec, final String prefix) {
        final Collection<TDataInput> inputs = ioSpec.getInputs()
                .stream()
                .map(i -> buildXmlDataInput(i, prefix))
                .collect(Collectors.toList());
        final Collection<TDataOutput> outputs = ioSpec.getOutputs()
                .stream()
                .map(o -> buildXmlDataOutput(o, prefix))
                .collect(Collectors.toList());

        final Collection<TInputSet> inputSets = ioSpec.getInputSets()
                .stream()
        ...
        final Collection<TOutputSet> outputSets = ioSpec.getOutputSets()
                .stream()
    ...
    private <T extends DataInputOutput, U> void buildXmlIOSet(
            ...
            final String prefix,
            final BiFunction<T, String, U> serializer1, final Function<U, JAXBElement<Object>> serializer2,
            final Supplier<? extends List<JAXBElement<Object>>> target
    ) {
        target.get().addAll(inputs.get()
                .stream()
