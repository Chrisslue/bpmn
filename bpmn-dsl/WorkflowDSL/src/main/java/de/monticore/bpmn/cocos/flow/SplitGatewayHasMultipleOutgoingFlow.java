 ...
 * Page: 289
 * Description: A Gateway MUST have either multiple incoming Sequence Flows or multiple outgoing Sequence Flows
 * (i.e., it MUST merge or split the flow).
 */
public class SplitGatewayHasMultipleOutgoingFlow implements WorkflowASTGatewayCoCo {

    @Override
    public void check(final ASTGateway gateway) {
        if (gateway.isDiverging() && gateway.sizeOutgoings() <= 1) {
            Log.error(Messages.get("0xWFM5001", gateway.getName()),
