 ...
 * Page: 181
 * Description: The list of BPMN elements that MUST be used in an Ad-Hoc Sub-Process: Activity.
 */
public class AdHocSubProcessContainsAtLeastOneActivity implements WorkflowASTSubProcessCoCo {
    ...
    public void check(final ASTSubProcess subProcess) {
        if (subProcess.isAdHoc() && WorkflowCollectors.toActivitiesLocal(subProcess).isEmpty()) {
            Log.error(Messages.get("0xWFM4002", subProcess.getName()),
                    subProcess.get_SourcePositionStart(), subProcess.get_SourcePositionEnd());
        }
    }
