...
public class ASTFlowTarget extends ASTFlowTargetTOP {
  ...
  public ListMultimap<ASTFlowElement, List<ASTFlowCondition>> asTarget() {
    ...
    if (isPresentElement()) {
      targetFlowNodes = ImmutableListMultimap.<ASTFlowElement, List<ASTFlowCondition>> builder()
          .put(resolveFlowNode(), isPresentCondition() ? Lists.newArrayList(getCondition()) : Lists
              .newArrayList()).build();
    }
    else if (isPresentGateway()) {
      targetFlowNodes = ImmutableListMultimap.<ASTFlowElement, List<ASTFlowCondition>> builder()
          .put(getGateway(), isPresentCondition() ? Lists.newArrayList(getCondition()) : Lists
              .newArrayList()).build();
    }
    else if (isPresentBlock()) {
