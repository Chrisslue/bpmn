...
public class ASTFlowTarget extends ASTFlowTargetTOP {
    ...
    public ListMultimap<ASTFlowNode, List<ASTFlowCondition>> asTarget() {
        ...
        if (isPresentNodeRef()) {
            targetFlowNodes = ImmutableListMultimap
                    .<ASTFlowNode, List<ASTFlowCondition>>builder()
                    .put(resolveFlowNode(), isPresentCondition() ? Lists.newArrayList(getCondition()) : Lists.newArrayList())
                    .build();
        } else if (isPresentNode()) {
            targetFlowNodes = ImmutableListMultimap
                    .<ASTFlowNode, List<ASTFlowCondition>>builder()
                    .put(getNode(), isPresentCondition() ? Lists.newArrayList(getCondition()) : Lists.newArrayList())
                    .build();
        } else if (isPresentBlock()) {
