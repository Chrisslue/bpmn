...
public class SubProcessFactory extends XmlFactory<ASTSubProcess, TSubProcess>
  ...
  public void visit(final ASTSubProcess subProcess) {
    switch (subProcess.getType()) {
      ...
      case ADHOC:
        {
          TAdHocSubProcess ta =
              create(factory::createTAdHocSubProcess, factory::createAdHocSubProcess);

          if (subProcess.isPresentAdHocCharacteristics()) {
            TAdHocOrdering ordering =
                subProcess.getAdHocCharacteristics().isSequential()
                    ? TAdHocOrdering.SEQUENTIAL
                    : TAdHocOrdering.PARALLEL;
