 ...
 * Page: 297
 * Description: If Message Intermediate Events are used in the configuration,
 * then Receive Tasks MUST NOT be used in that configuration and vice versa.
 */
public class EventGatewayDoesNotMixMessageEventsAndReceiveTasks implements WorkflowASTGatewayCoCo {
    ...
    public void check(final ASTGateway gateway) {
        if (gateway.getType().isEventBased()) {
            Collection<ASTEvent> messageEvents = gateway.streamOutgoings()
            ...
            Collection<ASTTask> receiveTasks = gateway.streamOutgoings()
                    .map(SequenceFlow::getTarget)
            ...
            if ((messageEvents.size() > 0) && (receiveTasks.size() > 0)) {
                messageEvents.forEach(event -> {
                    Log.error(Messages.get("0xWFM5008", gateway.getName()),
                            event.get_SourcePositionStart(), event.get_SourcePositionEnd());
                });
                messageEvents.forEach(task -> {
                    Log.error(Messages.get("0xWFM5008", gateway.getName()),
                            task.get_SourcePositionStart(), task.get_SourcePositionEnd());
                });
            }
