...
public class AddMoreImports extends WorkflowTransformation {
  ...
  protected void transform() {
    ...
    Collection<ASTMCImportStatement> astImports = imports.stream().map(this::createASTImport)
  ...
  private ASTMCImportStatement createASTImport(final ImportStatement imp) {
    ASTMCQualifiedName qualName = new ASTMCQualifiedNameBuilder().setPartsList(Arrays.stream(imp
        .getStatement().split("\\.")).collect(Collectors.toList())).build();
    return new ASTMCImportStatementBuilder().setMCQualifiedName(qualName).setStar(imp.isStar())
        .build();
  }
