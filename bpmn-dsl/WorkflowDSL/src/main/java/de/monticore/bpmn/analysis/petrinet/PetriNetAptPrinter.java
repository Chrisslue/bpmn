...
public class PetriNetAptPrinter {
    ...
    public String print(final ASTPetrinet net, final Map<ASTPlace, Long> marking) {
        ...
        // PRINT ARCS
        out.println(".flows");
        net.getTransitionList().forEach(transition -> {
            out.print(transition.getName());
            out.print(": ");

            out.print("{");
            out.print(transition.streamFromEdges().map(ASTEdge::getPlace).collect(Collectors.joining(", ")));
            out.print("}");

            out.print(" -> ");

            out.print("{");
            out.print(transition.streamToEdges().map(ASTEdge::getPlace).collect(Collectors.joining(", ")));
            out.println("}");
        });
        out.println();
        ...
        if (marking != null && !marking.isEmpty()) {
            ...
            final String markings = marking.entrySet().stream().map(e -> e.getValue() + "*" + e.getKey().getName())
