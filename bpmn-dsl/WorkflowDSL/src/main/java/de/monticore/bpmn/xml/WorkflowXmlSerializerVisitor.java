 ...
 *
 * <p>Extend this class to add vendor-specific or your own extensions, attributes, etc.
 *
 * @see WorkflowXmlSerializer
 */

// TODO include data and IO in XML output (mapping not completely clear, ignored by Camunda and
// other engines anyways ...)
...
public class WorkflowXmlSerializerVisitor extends WorkflowLocalVisitor {
  ...
  protected void handleSequenceFlow(final ASTFlowElementContainer container) {
    Map<SequenceFlow, TSequenceFlow> flows =
        WorkflowCollectors.toSequenceFlowLocal(container).stream()
            .collect(Collectors.toMap(Function.identity(), this::makeSequenceFlow));

    flows.keySet().stream()
        .filter(SequenceFlow::isDefault)
        .forEach(
            defaultFlow -> { // add default flow reference. bit hacky
  ...
  protected TSequenceFlow makeSequenceFlow(final SequenceFlow sequenceFlow) {
    JAXBElement<TSequenceFlow> xmlNode =
        CommonFactory.makeXmlSequenceFlow(
            sequenceFlow,
            getXmlNode(sequenceFlow.getSource()),
            getXmlNode(sequenceFlow.getTarget()));
    ...
    astNode
        ...
        .forEach(
            flow ->
                xmlNode
                    .getIncoming()
                    .add(new QName(WorkflowXmlUtils.getAsResourceKey(flow.getName()))));
    astNode
        .getOutgoingsList()
        .forEach(
            flow ->
  ...
  /**
   * Adds a flow node to the XML output Also takes care about adding the fow node to the current
   * lane (if any) and adding references to incoming/outgoing sequence flow.
   *
   * @param astNode
   * @param xmlNode
   */
  protected void addFlowNode(
      final ASTFlowNode astNode, final JAXBElement<? extends TFlowNode> xmlNode) {
    addFlowElement(xmlNode);
  ...
  /**
   * Adds a flow element to the XML output. For flow nodes, use {@link #addFlowNode(ASTFlowNode,
   * JAXBElement)}.
   *
   * @param xmlNode
   */
