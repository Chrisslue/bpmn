...
public class WorkflowScopesGenitor extends WorkflowScopesGenitorTOP {
  ...
  public IWorkflowArtifactScope createFromAST(ASTWorkflowCompilationUnit rootNode) {
    Log.errorIfNull(rootNode, "0xA7004x55587 Error by creating of the WorkflowScopesGenitor symbol table: top ast node is null");
    ...
    List<ImportStatement> imports = importStatements
      .stream()
      .map(importStatement -> new ImportStatement(importStatement.getQName(), importStatement.isStar()))
