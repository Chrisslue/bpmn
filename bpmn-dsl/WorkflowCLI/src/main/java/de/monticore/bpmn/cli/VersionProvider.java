 ...
 *
 * @see picocli.CommandLine.IVersionProvider implementation that returns version information from a {@code /build-info.properties} file in the classpath.
 */
public class VersionProvider implements de.monticore.bpmn.cli.CommandLine.IVersionProvider {
    public String[] getVersion() throws Exception {
        ...
        return new String[] {
                properties.getProperty("build.name") + " v" + properties.getProperty("build.version")
                + " (Build timestamp: " + properties.getProperty("build.timestamp") + ")"
        };
    }
}