...
public class WfConformanceChecker {
  ...
  public boolean checkConformance(ASTWorkflowCompilationUnit concrete,
      ASTWorkflowCompilationUnit reference, String mapping) {
    this.concrete = concrete;
    this.reference = reference;
    Log.info(String.format("Checking Conformance of [Concrete:%s] to [Reference:%s]\n", concrete
        .getWFProcess().getName(), reference.getWFProcess().getName()), "");
  ...
  public Set<CheckResult> getCheckResult() { return checkResult; }
  
  private boolean printCheckResults() {
    List<WfNode> nonConform = checkResult.stream().filter(n -> n.getResult().equals(
        CheckResult.Result.NON_CONFORM)).map(CheckResult::getNode).collect(Collectors.toList());
    
    List<WfNode> unKnown = checkResult.stream().filter(n -> n.getResult().equals(
        CheckResult.Result.UNKNOWN)).map(CheckResult::getNode).collect(Collectors.toList());
    ...
    for (CheckResult result : checkResult) {
      ...
      if (result.getResult().equals(CheckResult.Result.NON_CONFORM)) {
        ...
        Log.info(String.format("Result: Node [%s:%s] does not conform to Node [%s:%s]", con, result
            .getNode(), ref, refNode), "");
        String run;
      ...
      if (result.getResult().equals(CheckResult.Result.UNKNOWN)) {
        var refNode = this.incarnationStrategy.getReferenceElements(result.getNode()).get(0);
        Log.info(String.format("Result: Node [%s:%s] may not conform to Node [%s:%s]", con, result
            .getNode(), ref, refNode), "");
        Log.info(String.format(
  ...
  public List<WfNode> getNonConformNodes() {
    return checkResult.stream().filter(n -> n.getResult().equals(CheckResult.Result.NON_CONFORM))
        .map(CheckResult::getNode).collect(Collectors.toList());
  }
  
  public List<WfNode> getUnknownNodes() {
    return checkResult.stream().filter(n -> n.getResult().equals(CheckResult.Result.UNKNOWN)).map(
        CheckResult::getNode).collect(Collectors.toList());
  }
