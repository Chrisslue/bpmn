...
public class CTLConfStrategy implements ConformanceStrategy<WfNode> {
  ...
  public CTLConfStrategy(WfBuilder concrete, WfBuilder reference,
  ...
  public CheckResult checkConformance(WfNode concrete) {
    ...
    Set<WfNode> startNodes = this.concrete.getAllNodes().stream().filter(WfNode::isStart).collect(
        Collectors.toSet());
    
    Log.trace("", "");
    Log.trace(String.format("Checking Conformance of [%s] to [%s]", concrete, references.get(0)),
        "");
    ...
    BranchVisitor forwardVisitor = BranchVisitor.mkForwardVisitor(concrete, startNodes,
        postPredicate, incarnationStrategy);
    BranchVisitor backwardVisitor = BranchVisitor.mkBackwardVisitor(concrete, startNodes,
        prePredicate, incarnationStrategy);
