...
public class IDWfNodeBuilder implements WfBuilder {
  ...
  public void mkNamedTask(ASTWFTask task) {
    ASTStereotype stereotype = task.getModifier().isPresentStereotype() ? task.getModifier()
        .getStereotype() : null;
  ...
  public void mkNamedEvent(ASTWFEvent event) {
    ASTStereotype stereotype = event.getModifier().isPresentStereotype() ? event.getModifier()
        .getStereotype() : null;
  ...
  @Override
  public Set<WfNode> getAllNodes() { return new HashSet<>(allNodes.values()); }
  
  private void mkNode(String label, NodeType nodeType, ASTStereotype stereotype, boolean isStart,
      boolean isEnd) {
