...
public class BPMNConformanceTool extends WorkflowTool {
  ...
  public Options addAdditionalOptions(Options options) {
    ...
    Option reference = Option.builder("r").longOpt("reference").desc("Introduce the Reference BPMN")
        .numberOfArgs(1).build();
    options.addOption(reference);
    
    // introduce the reference BPMN
    Option concrete = Option.builder("c").longOpt("concrete").desc("Introduce the Concrete BPMN")
        .numberOfArgs(1).build();
    options.addOption(concrete);
    
    // introduce the reference BPMN
    Option map = Option.builder("m").optionalArg(true).longOpt("map").desc(
        "Introduce the Incarnation Mapping").numberOfArgs(1).build();
