...
public class PredicateBuilder {
  ...
  private static WfPredicate prePredicateRecursive(WfNode node) {
    ...
    if (node.getNodeType().isGateway()) {
      prevprev = node.getPredecessors().stream().map(PredicateBuilder::prePredicateRecursive)
  ...
  private static WfPredicate postPredicateRecursive(WfNode node) {
    ...
    if (node.getNodeType().isGateway()) {
      sucsuc = node.getSuccessors().stream().map(PredicateBuilder::postPredicateRecursive).collect(
          Collectors.toList());
    }
  ...
  public static WfPredicate mkXor(List<WfPredicate> formulas) {
    ...
    Predicate<List<WfNode>> combinedPredicate = nodes -> formulas.stream().filter(f -> f
        .getPredicate().test(nodes)).count() == 1;
    
    // Combine descriptions using Stream API
    String predicateString = formulas.stream().map(WfPredicate::toString).collect(Collectors
        .joining(" xor "));
