...
public class WorkflowGraphConverter extends WorkflowLocalVisitor {
   ...
   */
  public ImmutableGraph<ASTFlowElement> getGraph() { return ImmutableGraph.copyOf(graph); }
  ...
  public void visit(final ASTFlowElement flowNode) {
    ...
    flowNode.streamIncomings().map(SequenceFlow::getSource).forEach(source -> graph.putEdge(source,
        flowNode));
    flowNode.streamOutgoings().map(SequenceFlow::getTarget).forEach(target -> graph.putEdge(
        flowNode, target));
  }
