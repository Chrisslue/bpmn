...
public class AptChecker {
  ...
  public void check(final ASTWFProcess process) {
    ...
    final String apt = new PetriNetAptPrinter().print(wfNet.getPetriNet(), WorkflowNet
    ...
    try {
      ...
      final String output = new ProcessExecutor().command("java", "-jar", APT_PATH.toFile()
          .getAbsolutePath(), "strongly_live", aptFile.getAbsolutePath()).destroyOnExit()
          .readOutput(true).timeout(20, TimeUnit.SECONDS).execute().outputUTF8();
    }
