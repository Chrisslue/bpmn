...
public class PetriNetLoLaPrinter {
  ...
  public String print(final ASTPetrinet net, final Map<ASTPlace, Long> marking) {
    ...
    final String places = net.streamPlaces().map(ASTPlace::getName).collect(Collectors.joining(
        ", "));
    out.print(places);
    out.println(";");
    ...
    if (marking != null && !marking.isEmpty()) {
      final String markings = marking.entrySet().stream().map(e -> e.getKey().getName() + ": " + e
    ...
    net.streamTransitions().forEach(t -> {
      ...
      out.print("CONSUME ");
      final String incomingArcs = t.streamFromEdges().map(e -> e.getPlace() + ": " + e.getCount()
          .getValue()).collect(Collectors.joining(", "));
      out.print(incomingArcs);
      out.println(";");
      
      out.print("PRODUCE ");
      final String outgoingArcs = t.streamToEdges().map(e -> e.getPlace() + ": " + e.getCount()
          .getValue()).collect(Collectors.joining(", "));
