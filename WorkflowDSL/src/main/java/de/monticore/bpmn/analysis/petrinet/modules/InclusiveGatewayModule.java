...
public class InclusiveGatewayModule extends GatewayModule {
  ...
  public InclusiveGatewayModule(ASTWFGateway flowNode, List<ASTPlace> inputPlaces,
  ...
  private void initModule() {
    ...
    // OR-join
    Sets.powerSet(Sets.newHashSet(inputPlaces)).stream().filter(subset -> !subset.isEmpty())
        .forEach(subset -> {
    ...
    // OR-split
    Sets.powerSet(Sets.newHashSet(outputPlaces)).stream().filter(subset -> !subset.isEmpty())
        .forEach(subset -> {
