...
public class TimeIsValidCoCo implements TimerConditionsASTTimeCoCo {
  ...
  public void check(final ASTTime time) {
    try {
      ...
      Log.error(TimerMessages.TEMP.err("0xTEMP03", Joiner.on(":").skipNulls().join(time.getHours()
          .getSource(), time.getMinutes().getSource(), source)), time.get_SourcePositionStart());
    }
  }
