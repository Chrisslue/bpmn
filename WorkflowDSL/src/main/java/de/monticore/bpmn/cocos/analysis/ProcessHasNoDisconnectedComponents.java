...
public class ProcessHasNoDisconnectedComponents extends ProcessGraphCoCo {
  ...
  @Override
  public void check(final Graph<ASTFlowElement, EndpointPair<ASTFlowElement>> processGraph,
      final ASTWFProcess process) {
    if (process.isEmptyFlowElements()) { // allow empty processes (an empty process is disconnected by
      // definition)
      return;
    }
    ConnectivityInspector<ASTFlowElement, EndpointPair<ASTFlowElement>> inspector =
        new ConnectivityInspector<>(processGraph);
    List<Set<ASTFlowElement>> components = inspector.connectedSets().stream().filter(
        component -> !isCompensationActivity(component)).collect(Collectors.toList());
    
    if (components.size() > 1) {
      String formattedComponents = components.stream().filter(set -> set.size() > 1).map(set -> "{"
          + set.stream().map(ASTFlowElement::getName).sorted().collect(Collectors.joining(", "))
