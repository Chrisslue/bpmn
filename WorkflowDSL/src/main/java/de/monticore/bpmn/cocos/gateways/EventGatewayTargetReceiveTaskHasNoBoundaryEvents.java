...
public class EventGatewayTargetReceiveTaskHasNoBoundaryEvents implements WorkflowASTWFGatewayCoCo {
  ...
  public void check(final ASTWFGateway gateway) {
    if (gateway.getType().isEventBased()) {
      gateway.streamOutgoings().map(SequenceFlow::getTarget).flatMap(WorkflowFilters::isTask)
          .filter(task -> task.getType() == ASTConstantsWorkflow.RECEIVE).filter(
              receiveTask -> !receiveTask.isEmptyBoundaryEvent()).forEach(receiveTask -> Log.error(
                  Messages.get("0xWFM5009", receiveTask.getName()), receiveTask
                      .get_SourcePositionStart(), receiveTask.get_SourcePositionEnd()));
    }
  }
