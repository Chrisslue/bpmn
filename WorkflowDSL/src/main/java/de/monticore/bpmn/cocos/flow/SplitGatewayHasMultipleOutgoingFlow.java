 ...
 * split the flow).
 */
public class SplitGatewayHasMultipleOutgoingFlow implements WorkflowASTWFGatewayCoCo {
  
  @Override
  public void check(final ASTWFGateway gateway) {
    if (gateway.isDiverging() && gateway.sizeOutgoings() <= 1) {
      Log.error(Messages.get("0xWFM5001", gateway.getName()), gateway.get_SourcePositionStart(),
