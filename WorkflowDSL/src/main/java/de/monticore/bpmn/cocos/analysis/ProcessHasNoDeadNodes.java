...
public class ProcessHasNoDeadNodes extends ProcessGraphCoCo {
  ...
  protected void check(final Graph<ASTFlowElement, EndpointPair<ASTFlowElement>> processGraph,
    ...
    final Set<ASTFlowElement> startNodes = Sets.newHashSet(WorkflowCollectors.toStartNodesLocal(
        process));
    
    Sets.difference(processGraph.vertexSet(), new ReachabilityInspector<>(processGraph)
        .reachableFrom(startNodes)).forEach(node -> Log.error(Messages.get("0xWFM7009", node
            .getName()), node.get_SourcePositionStart(), node.get_SourcePositionEnd()));
  }
