 ...
 */
public class StartEventTopLevelProcessHasValidTrigger extends AbstractHasValidTriggerCoCo implements
  ...
  public void check(final ASTWFProcess process) {
    WorkflowCollectors.toEventsLocal(process).stream().filter(ASTWFEvent::isStart).forEach(
        this::check);
  }
