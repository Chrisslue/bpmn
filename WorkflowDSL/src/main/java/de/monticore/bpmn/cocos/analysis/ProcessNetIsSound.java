...
public class ProcessNetIsSound implements WorkflowASTWFProcessCoCo {
  ...
  public void check(final ASTWFProcess root) {
    ...
    if (WorkflowCollectors.toActivities(root).isEmpty()) {
      Log.info("Model does not contain any activities. Skipping soundness check.", getClass()
          .getSimpleName());
      return;
    }
    ...
    boolean soundTranslation = wfNet.getWarnings().stream().map(
        WorkflowNetConverter.Warning::getType).noneMatch(
            WorkflowNetConverter.Warning.Type.UNSOUND_TRANSLATION::equals);
    if (!soundTranslation) {
      Log.warn(Messages.get("0xWFM8002", root.getName()));
    }
    
    String lolaNet = new PetriNetLoLaPrinter().print(wfNet.getPetriNet(), WorkflowNet
        .initialMarking(wfNet));
    File lola;
    try {
    ...
    boolean hasNoNonInterruptingBoundaryEvents = WorkflowCollectors.toEvents(root).stream()
   ...
   */
  private void checkOptionToComplete(final WorkflowNet wfNet, final File lolaInput,
      final ASTWFProcess root, final Map<ASTPetriNode, Set<ASTFlowElement>> mapping) {
    ...
    try {
      boolean result = new LoLaChecker().formula(LoLaFormulae.completion(wfNet)).input(lolaInput)
          .check().getResult();
      if (!result) {
        isSound = false;
   ...
   * @param wfNet workflow-net to be checked
   */
  private void checkSafeness(final WorkflowNet wfNet, final File lolaInput, final ASTWFProcess root,
      final Map<ASTPetriNode, Set<ASTFlowElement>> mapping) {
    List<ASTFlowElement> unsafeNodes = wfNet.getPlaces().stream().filter(place -> {
      // LoLa manual - 4.2.4 k-boundedness of a place
      
      try {
        boolean result = new LoLaChecker().formula(LoLaFormulae.safe(place)).input(lolaInput)
            .check().getResult();
        return !result;
      }
      catch (TimeoutException e) {
      ...
      return false;
    }).map(mapping::get).filter(Objects::nonNull).flatMap(Collection::stream).distinct().collect(
        Collectors.toList());
    
    if (unsafeNodes.size() > 0) {
      isSound = false;
    }
    unsafeNodes.forEach(flowNode -> Log.warn(Messages.get("0xWFM8005", flowNode.getName()), flowNode
   ...
   * @param wfNet workflow-net to be checked
   */
  private void checkLiveness(final WorkflowNet wfNet, final File lolaInput, final ASTWFProcess root,
      final Map<ASTPetriNode, Set<ASTFlowElement>> mapping) { // no dead transitions
    List<ASTFlowElement> deadNodes = wfNet.getTransitions().stream().filter(transition -> {
      // LoLa manual - 4.2.8 Dead transition
      
      try {
        return new LoLaChecker().formula(LoLaFormulae.dead(transition)).input(lolaInput).check()
            .getResult();
      }
      catch (TimeoutException e) {
      ...
      return true;
    }).map(mapping::get).filter(Objects::nonNull).flatMap(Collection::stream).distinct().filter(
        flowNode -> !(flowNode instanceof ASTWFGateway)).collect(Collectors.toList());
    
    if (deadNodes.size() > 0) {
      isSound = false;
    }
    deadNodes.forEach(flowNode -> Log.warn(Messages.get("0xWFM8004", flowNode.getName()), flowNode
        .get_SourcePositionStart()));
  }
